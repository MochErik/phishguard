...
async def scan_url(request: Request, body: URLScanRequest):
    ...
    return await _build_url_response(body.url, ScanType.url, client_ip)
...
async def scan_qr(request: Request, file: UploadFile = File(...)):
    ...
    if is_url(decoded):
        client_ip = request.client.host if request.client else "127.0.0.1"
        return await _build_url_response(decoded, ScanType.qr, client_ip)
...
async def scan_web(request: Request, body: WebScanRequest):
    ...
    client_ip = request.client.host if request.client else "127.0.0.1"
    return await _build_url_response(body.url, ScanType.web, client_ip)
